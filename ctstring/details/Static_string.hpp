...
namespace CTString::Details
{
  ...
  public:
    ...
    c_str() const { return &value[0]; }

    string
    str() const { return {c_str()}; }

    auto
    begin() const { return value.begin(); }

    auto
    end() const { return prev(value.end()); }
    ...
    auto
    rbegin() const {return next(value.rbegin()); }

    auto
    rend() const {return value.rend(); }
  ...
  private:
    ...
    constexpr
    Static_string(T1 x1, T2 x2, Ts ... xs)
      : value{{x1, x2, xs ...}}
    {}

    constexpr
    Static_string(T x) : value{{{x}}}
    {
      static_assert(N == 0);
    }
    ...
    drop_aux(Idx<N>, Static_string const&, Ts ... xs){
      return Static_string<char_type, count_args(xs ...)>{
        xs ..., char_type(0)};
    }
    ...
    struct ConcatHelper<index_sequence<indices1 ...>, index_sequence<indices2 ...>>
    {
      template<integer M1, integer M2>
      static constexpr auto
      exec(Static_string<char_type,M1> const& str1, Static_string<char_type,M2> const& str2){
    ...
    friend constexpr auto
    concat(Static_string const& str1, Static_string<char_type,M> const& str2, U const& str3, Us const& ... strs){
    ...
    template<integer M>
    friend constexpr auto
    operator +(Static_string const& str1, Static_string<char_type,M> const& str2){
...
namespace std
{
  template<::CTString::Details::integer I, typename T, ::CTString::Details::integer N>
  constexpr T
